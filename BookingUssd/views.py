...
def ussdView(request):
    try:
        ...
        if node_name == "UpdateList":
            updates = Update.objects.filter(church=church,published=True,public=True).order_by('-datetime')
            counter = 1
            response = ""
        ...
        if node_name in ["UpdateDetail","FeaturedDetail"]:
            ...
            if node_name == "FeaturedDetail":
                ...
                update = Update.objects.filter(church=church,published=True,public=True).order_by('-datetime')[update_id]
                logger.info(update)
            if update.fetch_url:
